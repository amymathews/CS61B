...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
        ...
        double exp = numYears(targetYear);
        double x = 1 + rate/100;
        return Math.pow(x,exp)*presentValue;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double projectedVal = futureValue(presentValue,rate,targetYear);
        int exp = numYears(targetYear);
        double x =  1-inflationRate/100;
        return Math.pow(x,exp)*projectedVal;
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        for(int i = 0; i < period; i += 1){
            newTY = newTY - i;
            total += futureValueReal(perYear,rate,newTY,inflationRate);
        }
        return total;
    }
    ...
    static void printDollarFV(int targetYear, double returnRate,
                              double inflationRate) {
        double nominalDollarValue = futureValue(1.00,returnRate,targetYear); // replace 0 with your code
        double realDollarValue = futureValueReal(1.00,returnRate,targetYear,inflationRate);  // replace 0 with your code
